...
-- Calculate the position for the Load Script button
local loadScriptButtonYOffset = playerDropdown.Position.Y.Offset + playerDropdown.Size.Y.Offset + 10

-- Create the load script button
local loadScriptButton = Instance.new("TextButton")
loadScriptButton.Size = UDim2.new(0, 100, 0, 50)
loadScriptButton.Position = UDim2.new(1, -160, 0, loadScriptButtonYOffset)  -- Positioned directly below playerDropdown
loadScriptButton.Text = "Load Script"
loadScriptButton.Parent = screenGui

-- URL of the script to load
local scriptUrl = "https://raw.githubusercontent.com/liamspokemonner2/liamspokemonner2/main/DarkwareV2Battlegroundsscripttpup.lua"

local function loadExternalScript()
    local httpService = game:GetService("HttpService")
    local scriptSource = httpService:GetAsync(scriptUrl)
    local loadFunction = loadstring(scriptSource)
...
local toggleButtonYOffset = loadScriptButton.Position.Y.Offset + loadScriptButton.Size.Y.Offset + 10

-- Create the toggle button for teleportation
local toggleButton = Instance.new("TextButton")
toggleButton.Size = UDim2.new(0, 100, 0, 50)
toggleButton.Position = UDim2.new(1, -160, 0, toggleButtonYOffset)  -- Positioned below loadScriptButton
